...
def main() -> None:
    ...
    # NOTE: travel_cost_audit_report.xlsx is a MANUAL external dependency, not
    # generated by any pipeline script. It must be produced separately (e.g. from
    # a Jupyter notebook or spreadsheet analysis) and placed in the optimization
    # directory before running Step 10.
    if not AUDIT_XLSX.exists():
        sys.exit(f"ERROR: {AUDIT_XLSX} not found. This is a manual external file — see Step 10 docs.")
    ...
    nan_count = int(corrected["expected_cost_usd"].isna().sum())
    if nan_count > 0:
        raise ValueError(f"Corrected matrix contains {nan_count} NaN values!")
    neg_count = int((corrected["expected_cost_usd"] < 0).sum())
    if neg_count > 0:
        raise ValueError(f"Corrected matrix contains {neg_count} negative values!")
