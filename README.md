...
Step 11 pre-computes a per-(tech/candidate, node) cost table with drive/fly classification, rental car, and duration-scaled hotel costs. Hotel cost is `$159/night × hotel_nights`, where `hotel_nights` is derived from per-node average appointment duration. Day-trip logic zeros out hotel for short drive trips (≤150 mi haversine + ≤1 day avg duration). Toggle `FULL_COST_MODEL` in `scripts/config.py` to switch between full-cost and flight-only models.
...
- Best scenario: `N=0`
- N=0 annualized total: **$610,642** (travel $593,472 + overhead $17,170)
